...
# Show a summary with the number of total, completed, and pending tasks
def show_summary(tasks):
    total = len(tasks)                       # Total number of tasks
    done = sum(task["done"] for task in tasks)  # Count how many tasks are done
    pending = total - done                   # Pending tasks are total minus completed
    print(f"\nTotal tasks: {total}")         # Show total
    print(f"Completed: {done}")              # Show completed
    print(f"Pending: {pending}")             # Show pending
...
def main():
    ...
    while True:
        ...
        print("5. Show Summary")             # Option to show summary
        print("6. Quit")                     # Option to quit

        choice = input("Choose an option (1-6): ")  # Read user input
        ...
        if choice == "1":                    # Call add_task if user chose 1
            ...
            remove_task(tasks)
        elif choice == "5":                  # Call show_summary if user chose 5
            show_summary(tasks)
        elif choice == "6":                  # Exit program if user chose 6
